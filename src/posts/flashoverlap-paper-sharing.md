...
读论文前可以先看作者中文讲解的博客，好理解一些，然后再看论文：https://zhuanlan.zhihu.com/p/1897633068380054002
...
- 有依赖的计算和通信分为多个子操作，将子操作流水起来
- 例子：矩阵乘法产生张量 T

   - 分解为 N 个子矩阵乘法 分解为 N 个子张量 T1 ~ TN，针对 T 的一次通信分解为对 N 个子张量的 N 次子通信
   - 然后第 K 个矩阵乘法和第 K-1 个通信操作进行 Overlap

- 切分存在要求：切分的子张量必须显存连续，才能直接调用 NCCL
...
一、首先最直观和简单的设计是，每个 Tile 算完，我们就把结果发出来。
...
五、选一个 wave 作为 tile group 是不是最优选择
